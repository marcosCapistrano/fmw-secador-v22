...
static uint8_t check_sensor_entrada(state_manager_t state_manager)
{
    ...
    if (current > max)
    {
        ...
        if (current > max + 7 && !state_manager->is_aware_entrada)
        {
            ihm_send(ihm_update_queue, PAGE, TARGET_NONE, 9);
            result = 2;
        }
    }
    else if (current < min)
    {
        ...
        if (current < safe_subtraction(min, 7) && !state_manager->is_aware_entrada)
        {
...
static uint8_t check_sensor_massa_1(state_manager_t state_manager)
{
    QueueHandle_t peripherals_update_queue = state_manager->peripherals_update_queue;
    QueueHandle_t ihm_update_queue = state_manager->ihm_update_queue;
    ...
    if (current > max)
    {
        ...
        {
            ihm_send(ihm_update_queue, PAGE, TARGET_NONE, 11);
            result = 2;
        }
    }
    else if (current < min)
    {
        ...
        if (current < safe_subtraction(min, 7) && !state_manager->is_aware_massa_1)
        {
            ihm_send(ihm_update_queue, PAGE, TARGET_NONE, 12);
            result = 3;
        }
...
static uint8_t check_sensor_massa_2(state_manager_t state_manager)
{
    QueueHandle_t peripherals_update_queue = state_manager->peripherals_update_queue;
    QueueHandle_t ihm_update_queue = state_manager->ihm_update_queue;
    ...
    if (current > max)
    {
        ...
        if (current > max + 7 && !state_manager->is_aware_massa_2)
        {
            ihm_send(ihm_update_queue, PAGE, TARGET_NONE, 13);
            result = 2;
        }
    }
    else if (current < min)
    {
        ...
        if (current < safe_subtraction(min, 7) && !state_manager->is_aware_massa_2)
        {
            ihm_send(ihm_update_queue, PAGE, TARGET_NONE, 14);
            result = 3;
        }
    }
    else
...
static void state_add_event(state_manager_t state_manager, state_target_t event_type, uint8_t value)
{
    ...
    case SENSOR_MASSA_2:
        ...
        if (state_manager->event_last_massa_2 != value)
        {
        ...
        if (xQueueReceive(state_manager->state_manager_queue, (void *)&event, portMAX_DELAY))
        {
            ...
            case UPDATE:
                ...
                case SENSOR_ENTRADA:
                    ...
                    if (check_sensor_entrada(state_manager) >= 1 || check_sensor_massa_1(state_manager) >= 1 || check_sensor_massa_2(state_manager) >= 1)
                    {
                        if (state_manager->mode == 0)
                        {
                            perif_send(peripherals_update_queue, ACT, PERIF_QUEIMADOR, PERIF_RESP_NONE, 0);
                    ...
                    else if (check_sensor_entrada(state_manager) == 0 && check_sensor_massa_1(state_manager) == 0 && check_sensor_massa_2(state_manager) == 0)
                    {
                        if (state_manager->mode == 0)
                        {
                    ...
                    if (check_sensor_entrada(state_manager) >= 2 || check_sensor_massa_1(state_manager) >= 2 || check_sensor_massa_2(state_manager) >= 2)
                    {
                        perif_send(peripherals_update_queue, ACT, PERIF_BUZINA, PERIF_RESP_NONE, 1);
                    }
                    else if (check_sensor_entrada(state_manager) < 2 && check_sensor_massa_1(state_manager) < 2 && check_sensor_massa_2(state_manager) < 2)
                    {
                ...
                case IS_AWARE_ENTRADA:
                    state_manager->is_aware_entrada = true;
                    perif_send(peripherals_update_queue, ACT, PERIF_BUZINA, PERIF_RESP_NONE, 0);
                    ihm_send(ihm_update_queue, PAGE, TARGET_NONE, 3);
                    break;

                case IS_AWARE_MASSA_1:
                    state_manager->is_aware_massa_1 = true;
                    perif_send(peripherals_update_queue, ACT, PERIF_BUZINA, PERIF_RESP_NONE, 0);
                    ihm_send(ihm_update_queue, PAGE, TARGET_NONE, 3);
                    break;

                case IS_AWARE_MASSA_2:
                    state_manager->is_aware_massa_2 = true;
                    perif_send(peripherals_update_queue, ACT, PERIF_BUZINA, PERIF_RESP_NONE, 0);
                    ihm_send(ihm_update_queue, PAGE, TARGET_NONE, 3);
                    break;
